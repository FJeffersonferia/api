...
def login():
    ...
    cur.execute('SELECT * FROM users WHERE Usuario = %s AND Contraseña = %s', (username, password))
    user = cur.fetchone()
    cur.close()
    ...
    if user:
        ...
        session['Contraseña']=user[6]
        session['role'] = user[9]
        role = user[7]  # Obtener el rol del usuario
        return jsonify({'message': 'Login successful', 
                        'user': user[5],'Nombre':user[2],'Apellido':user[1],'username':user[5],'Contraseña':user[6],'Rol':user[7], 'role': role})
    else:
...
# Función para crear una intervención
...
def crear_intervencion():
    try:
        ...
        if not (fecha and estudiante and descripcion and tipo):
            return jsonify({'success': False, 'message': 'Datos incompletos'}), 400

        cur = mysql.connection.cursor()
        cur.execute('''
            INSERT INTO intervencion (Fecha, ID_estudiante, Descripcion, Tipo_de_intervencion)
            VALUES (%s, %s, %s, %s)
        ''', (fecha, estudiante, descripcion, tipo))
        mysql.connection.commit()
        cur.close()
...
def obtener_intervenciones():
    try:
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM intervencion JOIN estudiantes ON intervencion.ID_estudiante = estudiantes.ID_estudiante')
        ...
        for intervencion in intervenciones:
            content = {
                ...
                'estudiante': intervencion[2],
                'intervencion': intervencion[3],
                'detalle': intervencion[4]
            }
            payload.append(content)

        return jsonify(payload)
    except Exception as e:
        # Aquí se registra el error y se devuelve una respuesta adecuada
        print(f"Error en el servidor: {e}")
